...
const AdminPostcomponent = () => {
  ...
  const[totalnopost,settotalnopost]=useState([]);
  const[totalunapprovedpost,Settotalunapprovedpost]=useState([]);
...
const totalpost = async () => {     
  ...
  const responce = await fetch(apiurl + 'api/getaprovelpost')
      .then(responce => responce.json())
      .then(json => {
          const result = json.sort((a, b) => b.post_date.localeCompare(a.date))
          settotalnopost(result);
         var count=result.length;
            Settotalunapprovedpost(count); 
      })   
}
const Aprovel=(d)=>{
    axios.post(apiurl+"api/postapprovel/"+d).then((e)=>{
       ...
       <div className='container'>
        <aside>
          ...
          <div className='sidebar'>
            ...
            <div>
         <h1 className='posth1'>User Posts</h1>

         <div className='usercontainer'>
          <div className='top'>
          <h3 className='sidebarh2'>Total No Of UnApproved Post</h3>
              <h2>{totalunapprovedpost}</h2>
          </div>
         </div>
            ...
            <tbody className='tablebody'>
                {totalnopost.map((d,k)=>(
                    <tr key={k}>
                        ...
                        <td >{<img src={apiurl+d.photo} className='postphoto'></img>}</td>
